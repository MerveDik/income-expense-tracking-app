...
export const getCategories = () => async (dispatch: CategoryDispatch) => {     //fonksiyon oluşturuldu.
  dispatch({ type: "GET_CATEGORIES_START" });
  try {
    const response = await api().get<Category[]>("/categories");                  //apiye istek gönderip kategorilerimizi alıyoruz
    dispatch({ type: "GET_CATEGORIES_SUCCESS", payload: response.data });           // başarılı veya hatalı olma durumlarını dispatch ettik.
  } catch {
